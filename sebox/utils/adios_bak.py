...
def xmerge(node: Node, precond: float):
    ...
    _adios(node, 'xmerge_kernels smooth kernels_merged.bp')
    _adios(node, f'xprecond_kernels kernels_merged.bp {getdir().path("model_invstd.bp")} kernels.bp')
...
def xlbfgs(node: Node):
    ...
    for i in range(iter_min, node.iteration): # type: ignore
        if node.test_precond:
            lines.append(f'../iter_{i:02d}/postprocess/kernels_precond.bp')
        else:
            lines.append(f'../iter_{i:02d}/kernels.bp')
    ...
    # lines.append('kernels.bp')
    if node.test_precond:
        lines.append('postprocess/kernels_precond.bp')
    else:
        lines.append('kernels.bp')
...
def xupdate(node: Node, step: float, path_model: str, path_mesh: str):
    """Update model."""
    _adios(node, f'xprecond_kernels ../direction.bp {getdir().path("model_std.bp")} ../direction_std.bp')
    _adios(node, f'xupdate_model {step} {path_model} {path_mesh}/DATABASES_MPI/solver_data.bp ../direction_std.bp .')
